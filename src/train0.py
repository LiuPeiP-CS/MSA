...
def initiate(hyp_params, train_loader, valid_loader, test_loader):
    model = getattr(models, hyp_params.model+'Model')(hyp_params) # 获取类对象models里面的属性值得到一个函数，并将参数传递到该函数中
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def train(model, optimizer, criterion, ctc_a2l_module, ctc_v2l_module, ctc_a2l_optimizer, ctc_v2l_optimizer, ctc_criterion):
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            eval_attr = batch_Y.squeeze(-1) # if the number of labels is 1
            ...
            if ctc_criterion is not None:
                ...
                l_position = torch.tensor([i+1 for i in range(l_len)]*batch_size).int().cpu() # batch_size个[1, ..., l_len]
            ...
            if batch_chunk > 1:
                ...
                for i in range(batch_chunk):
                    ...
                    preds_i, hidden_i = net(text_i, audio_i, vision_i)
            ...
            torch.nn.utils.clip_grad_norm_(model.patameters(), hyp_params.clip)
            optimizer.step()
            ...
            if i_batch % hyp_params.log_interval == 0 and i_batch > 0:
                ...
                print('Epoch {:2d} | Batch {:3d}/{:3d} | Time/Batch(ms) {:5.2f} | Train Loss {:5.4f}'.format(
    ...
    def evaluate(model, xtx_a2l_module, ctc_v2l_module, criterion, test=False):
        model.eval()
    ...
    for epoch in range(1, hyp_params.num_epochs+1):
        ...
        print('-'*30)
        print('Epoch {:2d} | Time {:5.4f} sec | Valid Loss {:5.4f} | Test {:5.4f}'.format(epoch, duration, val_loss, test_loss))
        print('-'*30)

        if val_loss < best_valid:
            print('Saved model at pre_trained_models/{}.pt!'.format(hyp_params.name))
    ...
    if hyp_params.dataset == 'mosei':
        eval_mosei(results, truths, True)
    elif hyp_params.dataset == 'mosi':
        eval_mosi(results, truths, True)
    elif hyp_params.dataset == 'iemocap':
        eval_iemocap(results, True)
