...
def make_positions(tensor, padding_idx, left_pad):
    ...
    buf_name = "range_buf_{}".format(device)
...
class SinuPositionEmbedding(nn.Module):
    def __init__(self, embedding_dim, padding_idx=0, left_pad=0, ini_size=128):
        super(SinuPositionEmbedding, self).__init__()
    ...
    def forward(self, input):
        ...
        if device not in self.weights or max_pos > self.weights[device].size(0):
            self.weights[device] = SinuPositionEmbedding.get_embedding(max_pos, self.embedding_dim, self.padding_idx)
